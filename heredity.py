...
def compute_prob(people, one_gene, two_genes, have_trait, person, gene_count):
    if people[person]["father"] == None and people[person]["mother"] == None:
        prob = PROBS["gene"][gene_count]
    else:
        prob_inherit_father = 0
        prob_inherit_mother = 0
        if people[person]["father"] in one_gene:
            prob_inherit_father = 0.5
            prob_not_inherit_father = 0.5
        elif people[person]["father"] in two_genes:
            prob_inherit_father = 1 - PROBS["mutation"]
            prob_not_inherit_father = PROBS["mutation"]
        else:
            prob_inherit_father = PROBS["mutation"]
            prob_not_inherit_father = 1 - PROBS["mutation"]

        if people[person]["mother"] in one_gene:
            prob_inherit_mother = 0.5
            prob_not_inherit_mother = 0.5
        elif people[person]["mother"] in two_genes:
            prob_inherit_mother = 1 - PROBS["mutation"]
            prob_not_inherit_mother = PROBS["mutation"]
        else:
            prob_inherit_mother = PROBS["mutation"]
            prob_not_inherit_mother = 1 - PROBS["mutation"]

        if gene_count == 1:
            prob = (
                prob_inherit_father * prob_not_inherit_mother
                + prob_not_inherit_father * prob_inherit_mother
            )
        elif gene_count == 2:
            prob = prob_inherit_father * prob_inherit_mother
        else:
            prob = prob_not_inherit_father * prob_not_inherit_mother
...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in one_gene:
        prob *= compute_prob(people, one_gene, two_genes, have_trait, person, 1)
    for person in two_genes:
        prob *= compute_prob(people, one_gene, two_genes, have_trait, person, 2)
    for person in people.keys() - one_gene - two_genes:
        prob *= compute_prob(people, one_gene, two_genes, have_trait, person, 0)
    return prob
